...
namespace Game.Utils
{
	public sealed class Validate
	{
		...
		public static bool IsDouble(object expression)
		{
			if (expression != null)
			{
				return regex_IsDouble.IsMatch(expression.ToString());
			}
			return false;
		}
		...
		public static bool IsIDCard(string strIDCard)
		{
			...
			if (strIDCard.Length == 15)
			{
				...
				{
					string[] array = regex.Split(strIDCard);
					try
					{
		...
		{
			if (regex_IsNegativeInt.Match(expression).Success)
			{
				return long.Parse(expression) >= -2147483648;
			}
			return false;
		}
		...
		public static bool IsNumeric(object expression)
		{
			if (expression != null)
			{
				...
				if (text.Length > 0 && text.Length <= 11 && regex_IsNumeric.IsMatch(text) && (text.Length < 10 || (text.Length == 10 && text[0] == '1') || (text.Length == 11 && text[0] == '-' && text[1] == '1')))
				{
					return true;
				}
			}
		...
		{
			if (regex_IsPositiveInt.Match(expression).Success)
			{
				return long.Parse(expression) <= 2147483647;
			}
			return false;
		}
		...
		public static bool IsPositiveInt64(string expression)
		{
			if (regex_IsPositiveInt.Match(expression).Success)
			{
				return long.Parse(expression) <= 9223372036854775807L;
			}
			return false;
		}
